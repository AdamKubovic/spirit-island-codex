...
export function TierBoard({ initialSubject }: { initialSubject?: TierListSubject } = {}) {
  ...
  const visibleCardPool = hardFilter
    ? filterOwnedCards(items as PowerCard[], excluded, normalizeExpansion)
    : (items as PowerCard[])
  const groups =
    subject === 'configurations'
      ? groupByTier(visibleConfigurations, (c) => c.configId, tiers, viewed.tierLabels)
      : groupByTier(visibleCardPool, idOf as (item: PowerCard) => string, tiers, viewed.tierLabels)
